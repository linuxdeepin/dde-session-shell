...
public:
    ...
    explicit UserLoginWidget(const WidgetType widgetType = LoginType, QWidget *parent = nullptr);
    ~UserLoginWidget() override;

    void setModel(SessionBaseModel *model);
    void setUser(std::shared_ptr<User> user);

    void setWidgetType(const int type) { Q_UNUSED(type) }
...
private:
    ...
    void checkAuthResult(const int type, const int state);

private:
    SessionBaseModel *m_model; // 数据
    ...
    DLabel *m_passwordHintLabel;           // 密码提示
    ...
    AuthModule *m_activeDirectoryAuth;  // AD域
    AuthModule *m_fingerVeinAuth;       // 指静脉
